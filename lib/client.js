...
class DroneClient {
    ...
    getRepoByName(repoName) {
      return this._request('get', `/api/repos/${repoName}`).catch({statusCode: 404}, (err) => {
          return null;
      });
    };
    ...
    getRepo(owner, name) {
        ...
        return getRepoByName(`${owner}/${name}`)
    };
    ...
    createRepo(owner, name) {
        ...
        Joi.assert(owner, Joi.string().required(), 'Must specify owner');
        Joi.assert(name, Joi.string().required(), 'Must specify name');
        return this.createRepo2(`${owner}/${name}`);
    };

    createRepo2(repoName) {
      Joi.assert(repoName, Joi.string().required(), 'Must specify owner');
      return this._request('post', `/api/repos/${repoName}`);
    };

    updateRepo(repoName, repo) {
        Joi.assert(repo, Types.Repo, 'Invalid repo object');
        return this._request('patch', `/api/repos/${repoName}`, { payload: repo });
    };
